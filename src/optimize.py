...
class Alignment:
    ...
    def invoke_prompt(
        ...
        if self.client is None:
            error_message = "OpenAIError: The api_key client option must be set either by passing api_key to the client or by setting the OPENAI_API_KEY environment variable"
    ...
    def generate_revised_prompt(
        ...
        aws_result = self.generate_bedrock_response(revised_prompt, eval_model_id)
        pattern = r"<rewritten_prompt>(.*?)</rewritten_prompt>"
